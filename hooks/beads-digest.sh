...
echo 'BRANCH POLICY (beads-inited repos): work on a feature branch in a worktree → review the diff with a fresh subagent → push to `dev` → a HUMAN promotes dev → main (prod). NEVER push or merge to main yourself in a beads repo.'
